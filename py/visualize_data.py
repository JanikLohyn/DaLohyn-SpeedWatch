...
import pandas as pd
import matplotlib
matplotlib.use('Agg')  # Backend ohne GUI - WICHTIG für Threading!
...
try:
    ...
    plt.style.use('seaborn-darkgrid')  # Fallback für ältere Versionen
...
class SpeedwatchVisualizer:
    ...
    def plot_speed_over_time(self, save_path='graphs/speed_over_time.png'):
        ...
        if self.data is None or len(self.data) == 0:
            print("⚠️  Keine Daten vorhanden für speed_over_time")
            return
        ...
        try:
            ...
            plt.close(fig)  # Figure explizit schließen
        except Exception as e:
            print(f"❌ Fehler bei speed_over_time: {e}")
            plt.close('all')
    
    def plot_ping_over_time(self, save_path='graphs/ping_over_time.png'):
        """Erstellt Liniendiagramm: Ping über Zeit"""
        if self.data is None or len(self.data) == 0:
            print("⚠️  Keine Daten vorhanden für ping_over_time")
            return
    ...
    def plot_statistics(self, save_path='graphs/statistics.png'):
        ...
        try:
            ...
            print(f"❌ Fehler bei statistics: {e}")
            plt.close('all')
    
    def generate_all_graphs(self):
        """Erstellt alle Grafiken"""
        print("\n📊 Starte Grafik-Generierung...")
        
        if not self.load_data():
            print("❌ Kann keine Grafiken erstellen - keine Daten vorhanden")
            return False
